...
function checkData(data) {
  ...
  if (Object.keys(data).length !== 0) {
    ...
    try {
      ...
      throw new Error(`sec err`, { cause: err });
    } catch (err) {
...
function getAge(age) {
  ...
  try {
    ...
    err.message = "Вік не може бути менше 0!";
    err.name = "AgeError";
...
function getBookById(books, id) {
  ...
  // Перебираємо масив книг і шукаємо книгу з заданим ID
  // Перебираємо кожен елемент масиву книг
  for (let i = 0; i < books.length; i++) {
    if (books[i].id === id) {
      return `Книга: ${books[i].title}`;
    }
  }
...
function decodeURIComponentWrapper(encodedString) {
  ...
  try {
    let decod = decodeURI(encodedString);
    return decod;
  } catch (err) {
    return err.name;
  }
...
function findEvenNumber(numbers) {
  ...
  let err = Error(`У масиві немає чисел, що діляться на 2 без остачі!`);
  return err.toString();
}
  ...
  if (user !== undefined) {
    if (user.name) {
      if (user.email) {
        ...
        let err = Error(`Email користувача не вказано!`, { cause: user });
        console.log(`${err.message} ${JSON.stringify(err.cause)}`);
      }
    } else {
      console.log(
        Error(`Ім'я користувача не вказано!`, { cause: user }).message
      );
    }
  } else {
    console.log(Error(`Об'єкт користувача не вказано!`).message);
  }
}
...
function calculateSquareRoot(number) {
  ...
  if (typeof number === "number") {
    if (number > 0) {
      ...
      return Error(`Число не повинно бути від'ємним!`).message;
    }
  } else {
    return Error(`Аргумент має бути числом!`).message;
  }
}
...
function processData(data) {
  ...
  for (const index in data) {
    if (typeof data[index] !== "number") {
      return TypeError(`Елемент з індексом ${index} має бути числом!`).message;
    }
  }
